"""bot.py — Bot Telegram multi-cliente para carruseles"""
...
async def enviar_carrusel(update: Update, datos: dict, brand_color: str, platform: str):
    ...
    await update.effective_message.reply_text(f"📝 *Pie de foto:*\n{caption}\n\n#️⃣ *Hashtags:*\n{hashtags}", parse_mode="Markdown")
...
async def cmd_activar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        await update.message.reply_text("✉️ Escribe tu email:")
        return ASK_EMAIL
    tid = update.effective_user.id
...
async def recibir_nombre_marca(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎨 Elige color:", reply_markup=teclado_colores())
...
async def recibir_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎉 *¡Listo!*\n\nEnvíame texto, voz o foto para crear carruseles", parse_mode="Markdown")
    return ConversationHandler.END
...
async def _check_usuario(update: Update) -> tuple:
    ...
    if not ok:
        if razon == "not_active":
            await update.effective_message.reply_text("🔒 Necesitas suscribirte", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 Comprar €9", url=STAN_STORE_URL)]]))
        elif razon == "limit_reached":
            await update.effective_message.reply_text("📊 Límite alcanzado este mes")
    return ok, razon
...
async def seleccionar_plataforma(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await query.edit_message_text("⏳ Generando...")
    try:
...
async def procesar_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📱 Plataforma?", reply_markup=teclado_plataformas())
